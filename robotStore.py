...
class Product:
    def __init__ (self, name, price, quantities, checker):
        self.name= name
        self.price=price
        self.quantities=quantities
    def stock(self):
        for i in range(name,quantities):
            if procust[i].name in products[i].quantities
                return True
            return False 
    def cost(self):
        cost = product.price * product.quantities
        return (product.cost)
        print ("This product costs" + str(product.cost))
